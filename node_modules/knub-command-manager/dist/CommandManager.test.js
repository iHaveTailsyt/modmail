...
describe("CommandManager", () => {
    ...
    describe("Argument parsing", () => {
        ...
        it("Rest arguments", async () => {
            ...
            if (matched.error !== undefined)
                return chai_1.assert.fail();
            chai_1.expect(Object.keys(matched.args).length).to.equal(1);
            chai_1.expect(matched.args.arg.value).to.eql([20, 720]);
        });
        it("Options (default prefixes)", async () => {
            ...
            chai_1.expect(matched.opts.switch1.value).to.equal(true);
            chai_1.expect(matched.opts.switch2).to.be.undefined;
        });
        ...
        it("Custom option prefixes", async () => {
            ...
            if (matched.error !== undefined)
                return chai_1.assert.fail();
            chai_1.expect(matched.opts.option1.value).to.equal("optvalue1");
            chai_1.expect(matched.opts.option2.value).to.equal("optvalue2");
    ...
    describe("Argument/option types", () => {
        ...
        it("Custom types with context", async () => {
            ...
            if (matched1 === null)
                return chai_1.assert.fail();
            if (matched1.error !== undefined)
                return chai_1.assert.fail(matched1.error, undefined, `${matched1.error}`);
            chai_1.expect(matched1.args.arg1.value).to.equal(5);
            const matched2 = await manager.findMatchingCommand("!foo itjusthastobethere", { num: 20 });
            if (matched2 === null)
                return chai_1.assert.fail();
            if (matched2.error !== undefined)
                return chai_1.assert.fail(matched1.error, undefined, `${matched1.error}`);
            chai_1.expect(matched2.args.arg1.value).to.equal(20);
        });
        it("Async types", async () => {
            ...
            if (matched.error !== undefined)
                return chai_1.assert.fail();
            chai_1.expect(Object.keys(matched.args).length).to.equal(1);
            chai_1.expect(matched.args.arg.value).to.equal(50);
        });
        it("Async type for rest argument", async () => {
            ...
            if (matched.error !== undefined)
                return chai_1.assert.fail();
            chai_1.expect(Object.keys(matched.args).length).to.equal(1);
            chai_1.expect(matched.args.arg.value).to.eql([50, 1, 820]);
        });
    ...
    describe("Misc", () => {
        ...
        it("Should pass command config to command definition", async () => {
            ...
            chai_1.expect(command.config).to.eql({ aliases: ["bar"] });
        });
        ...
        it("Match correct overload/signature", async () => {
            ...
            if (!matched1 || manager.findMatchingCommandResultHasError(matched1))
                return chai_1.assert.fail();
            chai_1.expect(matched1.args.bar).to.exist;
            chai_1.expect(matched1.args.baz).to.not.exist;
            chai_1.expect(matched1.args.bar.value).to.equal(10);
            const matched2 = await manager.findMatchingCommand("!foo test");
            if (!matched2 || manager.findMatchingCommandResultHasError(matched2))
                return chai_1.assert.fail();
            chai_1.expect(matched2.args.bar).to.not.exist;
            chai_1.expect(matched2.args.baz).to.exist;
            chai_1.expect(matched2.args.baz.value).to.equal("test");
        });
        ...
        it("Original prefix exists", async () => {
            ...
            chai_1.expect(manager2.getDefaultPrefix().source).to.equal("^(?:a|b)");
